...
def test_variable_interpolation(mock_subproc, mock_shutil):
    ...
    def mock_open(file, *args, **kwargs):
        # If opening vdeps.toml, return a mock
    ...
    # Verify CMake Calls
    cmake_calls = [
        c for c in mock_subproc.call_args_list if "cmake" in c[0][0] and "-S" in c[0][0]
    ]
    assert len(cmake_calls) == 2, (
        "CMake configure should be called twice (Debug and Release)"
    )
    ...
    assert found_interpolated, (
        f"Did not find expected interpolated argument: {expected_arg}"
    )
...
def test_variable_interpolation_with_platform_prefix(mock_subproc, mock_shutil):
    ...
    assert found_linux, (
        f"Did not find expected linux interpolated argument: {expected_linux_arg}"
    )
    assert found_common, (
        f"Did not find expected common interpolated argument: {expected_common_arg}"
    )
    # Ensure win-prefixed option does NOT appear (platform mismatch)
    assert not any(arg.startswith("-DWIN_PATH=") for arg in args), (
        "Win-prefixed option should not appear on linux platform"
    )
